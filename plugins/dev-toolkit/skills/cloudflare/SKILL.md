...
#### Installing MCP Servers for Cursor Agent

This skill needs five Cloudflare MCP servers (see table above).

**Step 1 — Detect which servers are already installed and connected.** Use the mechanism Cursor exposes for inspecting MCP configuration (read `~/.cursor/mcp.json` and the project's `.cursor/mcp.json` if present; list the Cloudflare entries; note which are connected vs. needs-auth vs. absent). If you cannot determine this, ask the user once how to list MCP servers in Cursor — do **not** infer "MCP unavailable, fall back to API token."

**Step 2 — For any missing server, install it yourself** by editing the appropriate `mcp.json`. Do not print install commands for the user to copy. Each entry has the shape:
```json
"cloudflare-graphql": {
...
```
Only install the servers actually needed for the workflow you're about to run (see the "Used by" column).

**Step 3 — If any installed server is unauthenticated, hand off OAuth to the user.** Tell them to open Cursor's MCP settings, find the Cloudflare server(s) flagged *Needs authentication*, and complete the OAuth flow. Wait for confirmation. This is the only human step.

**Step 4 — Re-verify** that every required server reports connected and authenticated *and* that the `mcp__cloudflare_*` tools are actually loaded into your tool list before proceeding. If they are not loaded, the install did not take effect — ask the user to reload Cursor's MCP config (or restart Cursor) and wait. Do not proceed by guessing.
