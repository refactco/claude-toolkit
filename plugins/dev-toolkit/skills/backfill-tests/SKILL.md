...
Detection is a **single check**: does `apps/wordpress/tests/Unit/` exist? Don't hunt — there is exactly one tests directory and it is at the WordPress project root.
...
- **If `apps/wordpress/tests/Unit/` is absent**, scaffold the harness:
  ...
  - **Seed one smoke test so the suite isn't empty.** A freshly-scaffolded harness has zero tests, and PHPUnit exits **non-zero** on an empty suite (`No tests executed` / `Error: No tests found`) — so the green check below can't tell *passing* from *empty*. Create `apps/wordpress/tests/Unit/HarnessSmokeTest.php` with a single assertion (`assertTrue( function_exists( 'add_action' ) )`) so `npm run test:php` returns a clean `OK (1 test, 1 assertion)`. It's a throwaway: once the first Generated tests exist it can be deleted, and it is **not** part of the backfill commit (step 9).
- **If `apps/wordpress/tests/Unit/` exists**, do the new-machine bootstrap: run `npx wp-env start` in the background and wait for `✔ Done!`, then `npx wp-env run tests-cli --env-cwd=tests composer install`.
- **Guarantee the build artefacts are gitignored.** `composer install` creates `apps/wordpress/tests/vendor/`, the runner creates `.phpunit.result.cache`, and the coverage run creates `apps/wordpress/tests/coverage-html/` and `apps/wordpress/tests/coverage.xml` — **none must ever be committed**. Confirm `apps/wordpress/tests/.gitignore` exists and contains `/vendor/`, `.phpunit.result.cache`, `/coverage-html/`, and `coverage.xml`; **add whichever is missing.** Check this on both paths — the harness may predate the convention.
...
From `apps/wordpress`, discover maintained code by **reading `apps/wordpress/.gitignore` directly** — it uses negation patterns to explicitly allowlist everything the team owns:
...
Every `!wp-content/<base>/<name>` line is a maintained item; everything else is third-party. This is faster and more reliable than a `git check-ignore` loop, which must run from `apps/wordpress/` (not the repo root) or it queries the wrong `.gitignore`.

- Cover **subfolder** plugins/themes *and* **single-file** entries (e.g. `refact-gf-fingerprint.php` directly under `plugins/`).
- Use `git ls-files apps/wordpress/wp-content/<base>/<name> | head -1` only if a candidate's status is genuinely ambiguous (e.g. the `.gitignore` uses a glob and you're unsure whether a specific path is covered).
...
- **Location:** `apps/wordpress/tests/Unit/Generated/<Prefix><PascalName>/`. Prefix `Theme` | `MuPlugin` | `Plugin`; name in PascalCase. One folder per source folder.
