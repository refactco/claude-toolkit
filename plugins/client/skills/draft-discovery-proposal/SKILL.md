...
when_not_to_use: An existing engagement just needs a client deliverable, a meeting agenda, or an internal-only estimate not shown to the client.
next_skills: []
sub_agents: []
---
...
- Drop the draft in `docs/deliverables/` (or `docs/internal/` while still raw) with
  `status: draft` frontmatter — create the directory on demand if it doesn't exist yet.
  Promote status in place, don't move the file. A proposal is a client-facing deliverable.
