...
| ⚪ excluded | a test would add no *meaningful* safety — **(A)** restates the source, **(B)** already pinned by another row, or **(C)** low blast radius; note the reason (`⚪ excluded (low-impact: <why>)`). Canonical definition + patterns: the backfill-tests skill's `references/characterization-tests.md` §4 | yes |
