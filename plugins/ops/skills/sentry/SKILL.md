...
The script `${CLAUDE_PLUGIN_ROOT}/skills/sentry/scripts/sentry.mjs` does all the API calls, aggregation, and
...
node ${CLAUDE_PLUGIN_ROOT}/skills/sentry/scripts/sentry.mjs issues --json            # machine-readable
...
3. **Fix what's ours.** For each OURS issue, `issue <id>` to get the stack frames + captured
   vars, find the root cause, and make the change. Branch, commit, and open a PR for the
   fix using your normal git workflow.
